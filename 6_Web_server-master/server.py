...
def work(conn):
    ...
    send_good = f'HTTP/1.1 200 OK\nServer: SelfMadeServer v0.0.1\nDate: {t}\nContent-Type: text/html; charset=utf-8\nConnection: close\n\n'
    send_error = f'HTTP/1.1 404 OK\nServer: SelfMadeServer v0.0.1\nDate: {t}\nContent-Type: text/html; charset=utf-8\nConnection: close\n\n'
    ...
    if request == '/':
        with open('views/index.html', 'rb') as f:
            answer = f.read()
            conn.send(send_good.encode('utf-8')+answer)
    else:
        try:
            ...
            conn.send(send_error.encode('utf-8')+"Page not found".encode('utf-8'))
